...
namespace POFtpSender
{
    public partial class frmFtpSender : Form
    {
        ...
        {
            SetListViewText("Koneksi Internet tersedia");
            lvwFtp.Items.Add(_lstTeks[0]);
            SetListViewDB("Koneksi Internet tersedia");
            ...
            SetListViewText("Cek setting Aplikasi");
            lvwFtp.Items.Add(_lstTeks[0]);
            SetListViewDB("Cek setting Aplikasi");
            lvwDB.Items.Add(_lstTeksDB[0]);
            ...
            if (isSetting)
            {
                SetListViewText("Setting FTP berhasil...Load nama kolom");
                lvwFtp.Items.Add(_lstTeks[0]);
                _isFtpValid = true;
            }
            else
            ...
            if (isSettingDB)
            {
                SetListViewDB("Setting Database berhasil....Mulai mengambil data");
                lvwDB.Items.Add(_lstTeksDB[0]);
                _isDBValid = true;
            }
            else
            ...
            if (_isDBValid)
            {
                ...
                // ambil tanggal error
                SetListViewDB("Cek Tanggal Terakhir");
                lvwDB.Items.Add(_lstTeks[0]);
                GetLastDate();
                SetListViewDB("Tanggal transaksi terakhir : " + _LastErrDate.ToString("dd MMM yyyy"));
                lvwDB.Items.Add(_lstTeks[0]);
            }
        ...
        private void frmFtpSender_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            string ipAddress = FunctionHelpers.GetIP();
        ...
        private void bgwCekFile_DoWork(object sender, System.ComponentModel.DoWorkEventArgs e)
        {
            ...
            try
            {
                foreach (var fileItem in files)
                {
                    ...
                    switch (ClassHelper.jenisFile.ToUpper())
                    {
                        ...
                        case "XLS":
                            ...
                            catch (Exception ex)
                            {
                                ...
                                {
                                    SetListViewText("(" + fi.Name + ")" + " File dalam kondisi terbuka, tutup file terlebih dahulu.");
                                    _sbErrMessage.AppendLine(_lstTeks[0]);
                                    return;
                                }
                                ...
                                SetListViewText("(" + fi.Name + ")" + " ditolak karena file extension format bukan Excel (*.xls, *.xlsx)");
                                _sbErrMessage.AppendLine(_lstTeks[0]);
                        ...
                        case "TXT":
                            ...
                            {
                                SetListViewText("(" + fi.Name + ")" + " ditolak karena file extension format bukan Text File (*.txt)");
                                _sbErrMessage.AppendLine(_lstTeks[0]);
                                fi.MoveTo(failedPath + @"\" + DateTime.Now.ToString("ddMMyyHH24mmssfff") + "_" + fi.Name);
                                return;
                            }
                        ...
                        case "CSV":
                            ...
                            {
                                SetListViewText("(" + fi.Name + ")" + " ditolak karena file extension format bukan csv");
                                _sbErrMessage.AppendLine(_lstTeks[0]);
                                fi.MoveTo(failedPath + @"\" + DateTime.Now.ToString("ddMMyyHH24mmssfff") + "_" + fi.Name);
                                return;
                            }
                    ...
                    SetListViewText("File " + ClassHelper.jenisFile + " ditemukan.");
                    //Check nop is valid
                    ...
                    if (validNop.Count <= 0)
                    {
                        SetListViewText("(" + fi.Name + ")" + " ditolak karena nop tidak sesuai");
                        _sbErrMessage.AppendLine(_lstTeks[0]);
                        fi.MoveTo(failedPath + @"\" + DateTime.Now.ToString("ddMMyyHH24mmssfff") + "_" + fi.Name);
                        return;
                    }
                        ...
                        case "XLSX":
                            try
                            {
                                ...
                                if (dtPelaporan == null || dtPelaporan.Rows.Count <= 0)
                                {
                                    ...
                                    string namaFile = fInfo.Name.Replace(extens, string.Empty);
                            ...
                            {
                                SetListViewText("Read file excel gagal : " + ex.Message);
                                _sbErrMessage.AppendLine(_lstTeks[0]);
                                return;
                            }
                    ...
                    {
                        SetListViewText(errMsg);
                        _sbErrMessage.AppendLine(_lstTeks[0]);
                        return;
                    }

                    //Check bulan dan tahun pada datatable tidak boleh lewat tahun
                    ...
                    {
                        SetListViewText("Terjadi kesalahan, beberapa tanggal tidak pada bulan dan tahun yang sama.");
                        _sbErrMessage.AppendLine(_lstTeks[0]);
                        fi.MoveTo(failedPath + @"\" + DateTime.Now.ToString("ddMMyyHH24mmssfff") + "_" + fi.Name);
                        SetListViewText("File dipindahkan ke folder failed.");
                        _sbErrMessage.AppendLine(_lstTeks[0]);
                        return;
                    }
                    ...
                    using (var streamReader = new StreamReader(httpResApi.GetResponseStream()))
                    {
                        ...
                        {
                            SetListViewText("Data gagal diterima : " + pesan);
                            _sbErrMessage.AppendLine(_lstTeks[0]);
                        }
                    }
                    ...
                    if (httpResCP.StatusCode == HttpStatusCode.OK)
                    {
                        using (var streamReader = new StreamReader(httpResCP.GetResponseStream()))
                        {
                            ...
                            {
                                SetListViewText("Data tidak diterima, Nop dengan masapajak tersebut telah terbayar.");
                                _sbErrMessage.AppendLine(_lstTeks[0]);
                                fi.MoveTo(failedPath + @"\" + DateTime.Now.ToString("ddMMyyHH24mmssfff") + "_" + fi.Name);
                                SetListViewText("File dipindahkan ke folder failed.");
                                _sbErrMessage.AppendLine(_lstTeks[0]);
            ...
            {
                SetListViewText("Data gagal terkirim : " + ex.Message);
                _sbErrMessage.AppendLine(_lstTeks[0]);
            }
        }
        ...
        private void bgwSettingTime_RunWorkerCompleted(object sender, System.ComponentModel.RunWorkerCompletedEventArgs e)
        {
            if (_iTmrSearchUpload <= 0 && _iTmrSearchDB <= 0)
            {
                SetListViewText("Set timer tidak ditemukan");
                lvwFtp.Items.Add(_lstTeks[0]);
            }
        ...
        private void bgwGetData_RunWorkerCompleted(object sender, System.ComponentModel.RunWorkerCompletedEventArgs e)
        {
            ...
            {
                _LastErrDate = DateTime.Now;
                SetListViewDB("Tanggal Terakhir dikembalikan ke : " + _LastErrDate.ToString("dd MMM yyyy"));
                lvwDB.Items.Add(_lstTeks[0]);
            }
        ...
        private DataTable CsvToDatatable(string csvfilePath, char separator, out string errMessage)
        {
            ...
            string nop = fi.Name.ToUpper().Replace(".CSV", string.Empty);
        ...
        private bool ConfigurationValid(int maximumTryValue)
        {
            ...
            string clientIPAddress = FunctionHelpers.GetIP();
            while (!SenderTransaction.SendActivity(ClassHelper.userName, _urlApi, 0, "Inisialisasi Aplikasi", clientIPAddress, ClassHelper.cultureInfos, out _serverDate, out actMsg))
            {
                ...
                {
                    SetListViewDB(actMsg);
                    lvwDB.Items.Add(_lstTeksDB[0]);
                    SetListViewText(actMsg);
                    lvwFtp.Items.Add(_lstTeks[0]);
            ...
            if (!SerialKey.IsIdMachine(ClassHelper.userName, _urlApi, out errMsg))
            {
                ...
                sb.AppendLine("Lakukan setting ulang untuk username '" + ClassHelper.userName + "' ini.");
                ...
                SetListViewDB(sb.ToString());
                lvwDB.Items.Add(_lstTeksDB[0]);
                SetListViewText(sb.ToString());
                lvwFtp.Items.Add(_lstTeks[0]);
                return false;
            }
        ...
        private bool GetLastDate()
        {
            ...
            try
            {
                ...
                using (var streamReader = new StreamReader(httpLastErrorResponse.GetResponseStream()))
                {
                    ...
                    {
                        SetListViewDB("getLastDate gagal diterima");
                        _sbErrMessage.AppendLine(_lstTeks[0]);
                    }
                }
            }
            ...
            {
                SetListViewDB("getLastDate gagal eksekusi : " + ex.Message);
                _sbErrMessage.AppendLine(_lstTeks[0]);
            }
        ...
        private void SetListViewText(string commandTxt)
        {
        ...
        private void SetListViewDB(string commandTxt)
        {
        ...
        private DataTable ExcelToDatatable(string fileName, out string errMessage)
        {
            ...
            SetListViewText("Cek Kolom header");
            ...
            SetListViewText("Kolom Header ditemukan.");
            _sbErrMessage.AppendLine(_lstTeks[0]);
            SetListViewText("copy data dari Excel");
            _sbErrMessage.AppendLine(_lstTeks[0]);
        ...
        private DataTable TxtToDatatable(string fileName, char separator, out string errMessage)
        {
            ...
            string nop = fi.Name.ToUpper().Replace(".TXT", string.Empty);
            int iLines = 0;
            bool isHeaderFound = false;
        ...
        private void bgwGetData_DoWork(object sender, System.ComponentModel.DoWorkEventArgs e)
        {
            ...
            foreach (var item in lstDistJenPjk)
            {
                ...
                if (bodySource != null)
                {
                    ...
                    {
                        SetListViewDB(ex.Message);
                        _errGetDataMsg.AppendLine(_lstTeksDB[0]);
                        return;
                    }

                    SetListViewDB("Proses ambil data dari tanggal " + _LastErrDate.ToString("dd MMM yyyy") + " s/d tanggal " + _serverDate.ToString("dd MMM yyyy") + ".");
                    _errGetDataMsg.AppendLine(_lstTeksDB[0]);
                    ...
                    //string[] arrSelect = queryPajak.ToUpper().Replace(" AS ", string.Empty).Split(new string[] { " FROM " }, StringSplitOptions.None);
                    ...
                    try
                    {
                        ...
                        {
                            case "ORACLE":
                                DataParser.OracleDataParser(dtSource, queryPajak, queryLampiran, _LastErrDate, _serverDate, out dtPajak, out dtLampiran);
                                success = true;
                                break;
                            case "MYSQL":
                                DataParser.MySqlDataParser(dtSource, queryPajak, queryLampiran, _LastErrDate, _serverDate, out dtPajak, out dtLampiran);
                                success = true;
                                break;
                            case "SQL":
                                DataParser.SQLDataParser(dtSource, queryPajak, queryLampiran, _LastErrDate, _serverDate, out dtPajak, out dtLampiran);
                                success = true;
                                break;
                            case "ACCESS":
                                DataParser.AccessDataParser(dtSource, queryPajak, queryLampiran, _LastErrDate, _serverDate, out dtPajak, out dtLampiran);
                                success = true;
                                break;
                            default:
                                break;
                    ...
                    {
                        SetListViewDB("Gagal eksekusi query, " + ex.Message);
                        _errGetDataMsg.AppendLine(_lstTeksDB[0]);
                        string clientIPAddress = FunctionHelpers.GetIP();
                        string pesan = string.Empty;
                    ...
                    {
                        SetListViewDB("Data untuk perhitungan pajak ditemukan.");
                        _errGetDataMsg.AppendLine(_lstTeksDB[0]);
                    }
                    ...
                    if (!isNopFound)
                    {
                        SetListViewDB(!string.IsNullOrEmpty(errNop) ? "Nop tidak valid, " + errNop : "Nop tidak ditemukan");
                        _errGetDataMsg.AppendLine(_lstTeksDB[0]);
                        return;
                    }
                    ...
                    {
                        SetListViewDB("Nop : " + dtPajak.Rows[0]["NOP"].ToString() + ", Error like : " + ex.Message + ", jmlRowPajak" + dtPajak.Rows.Count);
                    ...
                    var namaKolom = "NOP";
                    //DataRow dtRow = dtLampiran.Rows[0];
                    //for (int iRow = 0; iRow < dtRow.ItemArray.Count(); iRow++)
                    //{
                    //    try
                    //    {
                    //        string strTes = dictAsNop[dtRow[iRow].ToString()];
                    //        namaKolom = dtLampiran.Columns[iRow].ColumnName;
                    //        break;
                    //    }
                    //    catch { }
                    //}                    
                    ...
                    if (dtPajak != null && dtPajak.Rows.Count > 0)
                    {
                        ...
                        if (string.IsNullOrEmpty(errMsg))
                        {
                            SetListViewDB(sccMsg);
                            _errGetDataMsg.AppendLine(_lstTeksDB[0]);
                        }
                        else
                        {
                            SetListViewDB(errMsg);
                            _errGetDataMsg.AppendLine(_lstTeksDB[0]);
                        }
                    }
                    else
                    {
                        SetListViewDB("Tidak ada data yang akan dikirim");
                        _errGetDataMsg.AppendLine(_lstTeksDB[0]);
                    }
                }
            }
        ...
        private List<ResponseSourceDB> GetSettingDatabase()
        {
            ...
            try
            {
                ...
                using (var streamReader = new StreamReader(httpResponse.GetResponseStream()))
                {
                    ...
                    {
                        SetListViewDB("Data gagal diterima, " + pesan);
                        _errGetDataMsg.AppendLine(_lstTeksDB[0]);
                    }
            ...
            {
                SetListViewDB("Data gagal diterima : " + ex.Message);
                _errGetDataMsg.AppendLine(_lstTeksDB[0]);
                //return;
            }
