...
namespace POFtpSender
{
    public partial class frmSettingDB : Form
    {
        ...
        private void btnGenerate_Click(object sender, EventArgs e)
        {
            ...
            ClassHelper.AliasTableSettingDB = new List<string>();
            if (!string.IsNullOrEmpty(tbParentAlias.Text))
            {
                ClassHelper.AliasTableSettingDB.Add(tbParentAlias.Text);
            }
            else
            {
                if (_indexCtrl > 0)
                {
                    ClassHelper.AliasTableSettingDB.Add(tbParentTable.Text);
                }
            }
            ...
            for (int iCtrl = 1; iCtrl <= _indexCtrl; iCtrl++)
            {
                ...
                if (!string.IsNullOrEmpty(tb.Text))
                {
                    ...
                    //string aliasRelation = string.IsNullOrEmpty(tbAlias.Text) ? string.Empty : "AS " + tbAlias.Text;

                    if (!string.IsNullOrEmpty(tbAlias.Text))
                        ClassHelper.AliasTableSettingDB.Add(tbAlias.Text);
                    else
                        ClassHelper.AliasTableSettingDB.Add(tb.Text);
