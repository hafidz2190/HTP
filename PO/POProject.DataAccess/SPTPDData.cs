...
namespace POProject.DataAccess
{
    ...
    {
        public static DataTable RetrieveDataSptpd(string username, int masapajak, int tahun, bool isActive)
        {
            ...
            cmd.Query = @"select ID_SPTPD, USERNAME, MASAPAJAK, TAHUN, PAJAK, SANKSI, TOTAL, ID_BAYAR, IS_ACTIVE 
                                from sptpd 
                            where username=:usern and masapajak=:masapajak and tahun =:tahun and is_active =:isactive";
    ...
    public class SPTPDDetailData
    {
        ...
        {
            OracleCmdBuilder cmd = DataBaseHelper.CreateOracleCommand();
            cmd.Query = @"select ID_SPTPD, NOP, USERNAME, MASAPAJAK, TAHUN, NOMINAL from sptpd_detail
